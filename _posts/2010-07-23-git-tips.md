...
The default, verbose `status` output is fine for beginners, but once you get proficient with git there is no need for it. Since I check the status fairly often, I want it to be as concise as possible.
...
The `cherry` command is useful to see which commits have been cherry-picked from a development branch to the stable branch, for instance. This command compares changes on the the current ("feature") branch to upstream ("master") and indicates which are present on both with the "-" sign. Changes still missing from upstream are marked with "+". 
...
[hub]: http://mislav.uniqpath.com/hub/